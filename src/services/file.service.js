...
// NOTE: on Windows often: gswin64c.exe
const ghostscriptPath = process.env.PDF_GS_PATH || "gs";

const pdfGsPreset = (process.env.PDF_GS_PRESET || "screen")
  .replace("/", "")
  .toLowerCase();
...
// Optional: strict PDF signature check (%PDF)
const ENABLE_PDF_HEADER_CHECK =
  String(process.env.PDF_HEADER_CHECK || "true").toLowerCase() === "true";
...
const sanitizeFileName = (name) =>
  String(name || "file.pdf")
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, "_")
    .replace(/\s+/g, "_")
    .slice(0, 180); // avoid too long names on windows
...
const ensureWithinBase = (base, targetPath) => {
  ...
  // allow base itself or children only
  // valid if rel === "" (same dir) OR rel doesn't start with ".." and isn't absolute
  const isInside =
...
const readPdfHeaderOk = async (filePath) => {
  // Reads first 4 bytes should be "%PDF"
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const base = path.basename(file.originalname || "file.pdf");
...
// Allow PDFs mainly by extension; mimetype may be wrong sometimes.
const fileFilter = (req, file, cb) => {
  const ext = path.extname(file.originalname || "").toLowerCase();
...
const buildGhostscriptArgs = (inputPath, outputPath) => {
  ...
  // optional: less noise but keep stderr
  args.push("-dQUIET");

  if (Number.isFinite(pdfGsColorDpi) && pdfGsColorDpi > 0) {
    args.push(
      "-dColorImageDownsampleType=/Average",
      `-dColorImageResolution=${pdfGsColorDpi}`,
    );
  }
  if (Number.isFinite(pdfGsGrayDpi) && pdfGsGrayDpi > 0) {
    args.push(
      "-dGrayImageDownsampleType=/Average",
      `-dGrayImageResolution=${pdfGsGrayDpi}`,
    );
  }
  if (Number.isFinite(pdfGsMonoDpi) && pdfGsMonoDpi > 0) {
    args.push(
      "-dMonoImageDownsampleType=/Subsample",
      `-dMonoImageResolution=${pdfGsMonoDpi}`,
    );
  }
...
const maybeOptimizeFiles = async (files, clientId, requestId) => {
  ...
  const optimized = await Promise.all(
    files.map(async (file) => {
      ...
      // Optional: verify the file is actually PDF
      if (ENABLE_PDF_HEADER_CHECK) {
        try {
          const ok = await readPdfHeaderOk(srcPath);
          if (!ok) {
            // eslint-disable-next-line no-console
            console.warn(
              `[pdf-check] Not a real PDF signature, fallback original: ${file.originalname}`,
            );
            return { ...file };
          }
        } catch (e) {
          // eslint-disable-next-line no-console
          console.warn(
            `[pdf-check] Failed reading header, fallback original: ${file.originalname}`,
          );
          return { ...file };
        }
      }
      ...
      const destPath = path.join(
        optimizedDir,
        file.filename || path.basename(srcPath),
      );

      try {
        await optimizePdf(srcPath, destPath);
        return { ...file, archivePath: destPath };
      } catch (err) {
        // eslint-disable-next-line no-console
        console.warn(
          `[pdf-optimize] fallback to original for ${file.originalname}: ${err.message}`,
        );
        return { ...file };
      }
    }),
...
const archiveFiles = async ({ clientId, archiveId, files }) => {
  ...
  const clientArchiveDir = ensureWithinBase(
    archiveBaseAbsolute,
    path.join(archiveBaseAbsolute, clientId),
  );
  await fsp.mkdir(clientArchiveDir, { recursive: true });
  ...
  return new Promise((resolve, reject) => {
    ...
    files.forEach((file, idx) => {
      ...
      // sanitize & make unique inside zip
      const original = path.basename(file.originalname || "file.pdf");
      const safe = sanitizeFileName(original);
      const zipName = `${String(idx + 1).padStart(2, "0")}-${safe}`;
...
const resolveArchivePath = (storedPath) => {
  // storedPath may already be absolute; resolve handles both
  const absolute = path.resolve(process.cwd(), storedPath);
  return ensureWithinBase(archiveBaseAbsolute, absolute);
};
